...
with open('intents.json', 'r') as f:
    ...
    for intent in intents['intents']:
        ...
        for pattern in intent['patterns']:
            w = tokenize(pattern)
            all_words.extend(w)
            xy.append((w , tag)) #patterns and corresponding tag in xy
    ...
    for (pattern_sentence, tag) in xy:
        ...
        label = tags.index(tag) #get number for label
    ...
    class ChatDataset(Dataset):
        def __init__(self):
            ...
            self.x_data = X_train
            self.y_data = y_train
        #retrieve dataset with an index 
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1 ) % 100 == 0:
            print (f'Reading data, training module...  {epoch+1}/{num_epochs}, loss probability={loss.item():.4f}')
